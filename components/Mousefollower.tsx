  ...
  useEffect(() => {
    ...
    function spawnStar(x: number, y: number) {
      ...
      requestAnimationFrame(() => {
        ...
        star.style.transform = `translate(calc(-50% + ${dx}px), calc(-50% + ${dy}px)) scale(0)`;
    ...
    const loop = () => {
      ...
      glow.current.x += (mouse.current.x - glow.current.x) * 0.1;
      glow.current.y += (mouse.current.y - glow.current.y) * 0.1;
    ...
    <>
      ...
      <div
        ...
        style={{
          ...
          borderRadius: "50%",
          background:
            "radial-gradient(circle, rgba(201,98,47,0.18) 0%, transparent 70%)",
          filter: "blur(4px)",
      ...
      <div
        ...
        style={{
          ...
          boxShadow:
            "0 0 6px rgba(255,255,255,0.6), 0 0 12px rgba(201,98,47,0.4)",
          opacity: 0, // revealed via direct DOM write on first mousemove
          transition: "opacity 0.3s ease",
        }}
        aria-hidden="true"
